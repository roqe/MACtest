...
TEGSS<-function(
  ...
  Q.half=sapply(1:n,function(i){ (X[i]*t(Y1[,i])%*%v.work.inv)^2 })
  Q=sum(Q.half)
  LD=lapply(1:n,function(i){
    return(diag(X[i],nrow = nrow(v.work))%*%v.work.inv%*%diag(X[i],nrow = nrow(v.work)))
  })
  LL=Reduce("+",LD)
  LR=round(eigen(LL)$values,digits = 2)
  p_davs=CompQuadForm::davies(Q,LR)$Qq
  p_perm=p_rsch=NA
  ...
  if(!is.na(n.perm)){
    ...
    p_rsch<-pchisq(Q*2*mean(Q.perm.sub)/var(Q.perm.sub), df=2*mean(Q.perm.sub)^2/var(Q.perm.sub), lower.tail=F)
  }
  ...
  ## Output the results
  A=list(stru=work2,matx=v.work,p_rsch=p_rsch,p_perm=p_perm,p_davs=p_davs)
  return(A)
}
