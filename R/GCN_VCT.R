...
GCN_VCT<-function(S, M, Y, X, PS=NULL, mc=5, prec=1e-3){
  GS=parallel::mclapply(1:length(Y),function(c){
    ...
    if (all(Y[[c]] %in% 0:1)) obj<-SKAT::SKAT_Null_Model(Y[[c]]~-1+X+S, out_type="D")
    if (!all(Y[[c]] %in% 0:1)) obj<-SKAT::SKAT_Null_Model(Y[[c]]~-1+X+S, out_type="C")
    pb<-SKAT::SKAT(M[[c]], obj, is_check_genotype=FALSE, kernel="linear")$p.value
